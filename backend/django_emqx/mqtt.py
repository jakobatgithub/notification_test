...
class MQTTClient:
    ...
    def __init__(self, broker, port=1883, keepalive=60):
        ...
        for attempt in range(settings.DJANGO_EMQX_SETTINGS['MAX_RETRIES']):
            try:
                ...
                print(f"⏳ Connection refused, retrying in {settings.settings.DJANGO_EMQX_SETTINGS['RETRY_DELAY']} seconds...")
                time.sleep(settings.settings.DJANGO_EMQX_SETTINGS['RETRY_DELAY'])
